...
# The line below creates a new file 'object' named f that is linked to the Input Code Files\Examples\example.txt file in this folder.

f = open('Input Code Files\Examples\example.txt', 'r+')


# This means f is open for reading.
# The first argument (Input Code Files\Examples\example.txt) is the filename and the second argument is the mode.
# The mode determines the mode in which the file has to be opened.

# Here we intend to read and write from/to Input Code Files\Examples\example.txt, which is already in the same folder as this file, example.py.
# Python will look in this directory for 'Input Code Files\Examples\example.txt', and try read its context.
...
print("\nExample 3: ")
contents = ""
with open('Input Code Files\Examples\example.txt', 'r+') as f: # Open the file again, this time using the 'with' syntax
        for line in f:
...
f = open('Input Code Files\Examples\example.txt','r+', encoding='utf-8') # Open the file again!
